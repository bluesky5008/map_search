...
- **백그라운드·무인 동작:** 게임 창이 다른 창에 가려져 있어도 스캔이 진행됩니다(Windows Graphics Capture + PostMessage, 실기 검증 완료). **모니터 off + 원격(RDP) 연결 해제 상태의 완전 무인 운용도 실증**됐습니다(8행 연속 완주). 창 **최소화**와 **시스템 절전·로그아웃**은 안 됩니다. 상태 점검은 `spikes/s7_mumu/health_check.py`(읽기 전용).

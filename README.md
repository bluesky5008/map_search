...
> **상태 (2026-08-02):** 요구사항·설계 기준선 **v2 승인**. 구현 진행 중 — 저장 계층·창/캡처/입력 계층 완료, 다음은 인식(vision) 계층.
...
| **MODE-A2** | **디테일 줌 스캔** | **A1 전체 + 자원 종류·공터·1칸건물·2칸이상건물** | **약 10~15시간** | **v1** |
...
현재는 창/캡처/입력 계층 점검 명령만 있습니다. 스캔 명령은 T13에서 추가됩니다.

```powershell
# 대상 창 목록
$env:PYTHONPATH = "src"; .venv\Scripts\python -m mapscan.cli windows
...
```powershell
$env:PYTHONPATH = "src"; .venv\Scripts\python -m unittest discover -s tests
```
...
│   ├── design-change-log.md     # 설계 변경 이력
│   ├── changes/DCR-001-*.md     # 설계 변경 요청 (승인됨)
│   └── decisions/ADR-001~005    # 설계 결정 기록
...
| [design-change-log.md](docs/work/20260801-worldmap-land-scan/design-change-log.md) | 설계 변경 이력 (DCR) |
| [ADR-001](docs/work/20260801-worldmap-land-scan/decisions/ADR-001-tech-stack.md) | 구현 스택: Python + OpenCV |
| [ADR-002](docs/work/20260801-worldmap-land-scan/decisions/ADR-002-capture-input.md) | 캡처·입력: WGC + PostMessage (스파이크 통과) |
...
| [ADR-005](docs/work/20260801-worldmap-land-scan/decisions/ADR-005-window-aspect-ratio.md) | 창 크기·종횡비 정책 |
...
1. **v1 (진행 중):** MODE-A2 디테일 줌 전체 스캔 — 저장·창/캡처/입력 계층 완료, 인식·순회·컨트롤러 남음

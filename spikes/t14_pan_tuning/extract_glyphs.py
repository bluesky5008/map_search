...
SOURCES = [
    ...
    # AC-03 지정 영역 스캔(행 148~157, 서량 사막 지역)의 재앵커 실패 증거 —
    # 이 지역 렌더링에서 '('가 일관되게 '1'로 오매칭(판독 성공 2/27,
    # read_evidence.py 코퍼스). 일괄 수확은 정렬 오차 시 오라벨 템플릿을 만들어
    # 롤백됐으므로(ev_r151d에서 한 클러스터 밀림 실측) 정렬 신뢰도가 높은
    # 소스(8/9 일치)에서 '(' 변형만 수확한다. 나머지 ev_*는 read_evidence.py의
    # 평가 코퍼스로 유지.
    ("ev_r148b.png", "(240,680)"),
    ("ev_r149a.png", "(36,890)"),
    ("ev_r150b.png", "(71,863)"),
    ("ev_r151e.png", "(206,733)"),
    # 지정 영역 스캔 잔존 실패(58장 코퍼스) 중 '('→'1' 패턴 + 로그 예측
    # 허용창 정합 소스만 추가 수확(2026-08-02 2차). 진실은 원시 판독의
    # 결손('(' 1글자)을 예측 창으로 확증 — mx 자릿수 미상 소스는 제외했다.
    ("ev_r149c.png", "(71,857)"),   # 예측 (76,846)±20
    ("ev_r150c.png", "(71,863)"),   # 예측 (76,852)±20
    ("ev_r153a.png", "(71,880)"),   # 예측 (72,875)±11
    ("ev_r162a.png", "(70,937)"),   # 예측 (72,929)±20
    ("ev_r163a.png", "(405,607)"),  # 예측 (406,599)±20
    # ev_r148a('8')·ev_r152a('0')는 수확 시 교차 오판독으로 재검증 롤백됨 —
    # 다지점 변형('8_2', '0_4')은 다른 글리프를 잠식한다. 소스에서 제외
]

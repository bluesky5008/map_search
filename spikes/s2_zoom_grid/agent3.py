"""S-2c elevated agent v3: agent2 + setrect (exact position/size, for snap quadrants).

Extra command:
  setrect <x> <y> <w> <h>   move+resize outer window (bypasses aspect-ratio drag limits)
Others: cap/click/move/wheel/resize/rect/sleep/quit (see agent2.py)
"""
...
class Agent:
    ...
    def execute(self, line: str) -> str:
        ...
        if op == "setrect":
            ...
            rc = user32.SetWindowPos(self.hwnd, 0, x, y, w, h, 0x0004 | 0x0010)
            return f"setrect rc={rc} -> {self.rect()}"
        if op == "resize":
            w, h = int(parts[1]), int(parts[2])
            rc = user32.SetWindowPos(self.hwnd, 0, 0, 0, w, h, 0x0002 | 0x0004 | 0x0010)
            return f"resize rc={rc} -> {self.rect()}"
        if op == "rect":
            return self.rect()
        if op == "sleep":

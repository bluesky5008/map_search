...
| **다음 작업** | **T14.3 글리프 보강 우선**(팝업 좌표 "3"↔"8" 오판독이 결정적으로 재발 — 재앵커 성공률에 직결, 확정 사실 32) → 점령형·내땅 표본 수집, FR-07 병합, AC-01/02/03/05/06/07 기록 → 지정 영역(100×100) 스캔 → 전체 스캔(착수 전 사용자 확인) → NFR-02 판정 |
| 테스트 | `python -m unittest discover -s tests` — **84건** 전부 통과 (환경 주의: `.venv`에 `pip install -e . --no-deps` 필요) |
...
| 32 | 팝업 좌표 오판독은 **타일별로 결정적**이다("3"→"8"이 같은 타일에서 반복 — 행 150 팬 #12와 종료 재앵커 모두). 새니티 창이 차단하지만 그 사이클 꼬리(~216타일)가 폐기된다 | T14.3 글리프 보강이 재앵커 성공률·보충 부담에 직결. 오판독 표본은 res_0007 로그의 (138,790)→(187,793) 지점 |

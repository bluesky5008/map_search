...
| 스파이크 | S-1·S-2 완료. **S-3 완료**(입력란 조작·클램프 감지), **S-4 완료**(팝업은 닫을 수 없음 → 겹침 커버 유지), **S-5 완료·통과**(드래그 팬 실현 가능 — 아래 확정 사실 15~19, `spikes/s5_drag_pan/findings.md`) |
| 구현 완료 | T8 골격, T10 store, T9 win, T11 vision, **T12 nav**, 부가 도구 |
| **다음 작업** | **DCR-002 안 (d) 최적화 구현** — ① 이동을 행 시작 점프 + 행 내 좌팬으로 전환 ② 분류를 신규 커버 대역으로 한정 ③ HUD 사각형 축소·팝업 사각형 재실측 → **방문 100회 연속 실측**으로 방문당 소요·커버 타일 산출 → NFR-02 확정 DCR 발행 |

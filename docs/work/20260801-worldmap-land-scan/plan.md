...
| 구분 | 상태 |
|---|---|
| 요구사항·설계 | v2 승인. **NFR-02(스캔 소요 하루 이내)가 실측과 충돌 — 사용자 결정 필요(아래 R-09)** |
| 스파이크 | S-1·S-2 완료. **S-3 완료**(입력란 조작·클램프 감지), **S-4 완료**(팝업은 닫을 수 없음 → 겹침 커버 유지) |
| 구현 완료 | T8 골격, T10 store, T9 win, T11 vision, **T12 nav**, 부가 도구 |
| **다음 작업** | **T13 controller/cli** (ScanController + DetailScan, Watchdog, 진행률·재개). 단, R-09 결정이 스캔 전략에 영향 |
...
- 관련 DCR: [DCR-001](changes/DCR-001-mode-a-zoom-limits.md) 승인 — v1 구현 범위는 **MODE-A2만** / **[DCR-002](changes/DCR-002-scan-throughput.md) 제안됨 — 스캔 소요 NFR-02 미달, 결정 대기**
...
- [ ] T7b. S-5 드래그 팬 실현 가능성 — DCR-002 (d)안의 착수 게이트 (미수행)
...
- [ ] T13. controller/cli — ScanController + DetailScan(A2), Watchdog, 진행률·재개. A1/B/C는 스텁. **DCR-002 결정에 따라 이동 전략(점프 vs 팬)이 달라지므로 착수 전 결정 필요**
...
→ **사용자 결정 필요.** 선택지: (a) NFR-02 완화(예: 2~3일) (b) v1 범위를 MODE-A1(전략 뷰, 자원 종류 없음)로 축소 (c) 영역 한정 스캔으로 축소 (d) 최적화 후 재측정하여 판단.

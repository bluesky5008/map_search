...
- 기준선: **v3 (승인됨)** — 승인일 2026-08-02 ([DCR-003](changes/DCR-003-nfr02-final-pan-design.md): 팬 이동 전략 + NFR-02 확정. 이전: v2 [DCR-001](changes/DCR-001-mode-a-zoom-limits.md))
- 대상 요구사항: [requirements.md](requirements.md) v3
- 작성일: 2026-08-01 / 최종 갱신: 2026-08-02 (S-5/S-6 실측 반영, DCR-003 승인)
...
| GridMapper | 화면 픽셀 ↔ 맵 좌표 변환(**일반 평행사변형 격자**, 기저 `E_MX`/`E_MY`). 앵커는 점프 후 대상 타일이 놓이는 **고정 화면 위치**. 가시 셀 열거 시 오클루전 사각형과의 교차를 분리축 정리로 판정. ⚠ **기저는 국소값**(지면이 원근 틸트로 렌더링, S-5) — 셀 좌표 계산은 앵커 y 근방 대역으로 한정한다 | 캘리브레이션 설정(`nav/ui.py`) |
...
### 4.3 MODE-A2 스캔 루프 (v1, DCR-003 행 기반)
...
for row in planner.resume_from(checkpoint):
    ...
    classify_new_cells(frame, band, exclude=HUD+popup)   # 행 시작 방문(팝업 오클루전 있음)
    ...
    for k in range(row.pans):                      # 행 내 팬 방문
        ...
        A += a; B += b                             #   기대 게인 창 밖 밴드는 기각
        classify_new_cells(frame, band, shift=(A, B))    # 대역 내 신규 진입 셀만
...
- **팬 이동 추적(S-5 발견 3):** 지면이 원근 틸트로 렌더링되어 **팬 이동량이 화면 y에 선형 의존**한다(전단 이동장). 전폭 밴드 3개의 x 이동을 `dx(y)=a+b·y`로 적합해 누적하고, 셀 위치는 대역별 이동량으로 갱신한다. 눈(겨울) 지형은 추적 NCC가 낮아 간헐 오추적이 있으므로 **기대 게인 창(±80px) 밖 밴드를 기각**하고 2밴드 적합으로 대체한다(S-6 관찰 1). 팬 게인은 드래그 길이에 따라 달라 캘리브레이션이 아니라 매 팬 실측으로 다룬다.
...
- **오클루전 처리(P-06):** HUD 사각형은 실측 불투명 영역으로 축소했다(S-6 `HUD_REFINED` → `nav/ui.py` 반영). **선택 팝업은 행 시작 프레임에만 존재**하고 첫 팬에서 사라진다(S-5 G1) — 팝업 사각형(타일 중심 상대 −290,−215 ~ +370,+150 실측)은 행 시작 방문에서만 제외한다. 팝업은 닫을 수 없다(S-4). 스캔 종료 시 미커버 좌표가 남으면 보충 방문을 생성한다(AC-05).
...
- **성능(NFR-02) — S-6 재측정으로 확정([DCR-003](changes/DCR-003-nfr02-final-pan-design.md)):** 행 기반 팬 스캔 방문 99회 실측 — **광폭 팬(1900px) 방문당 2.95초·신규 68.2타일**(23.1타일/s), 행 시작 점프 6.70초·51타일. 맵 1620² 외삽 **33~36시간**(행당 팬 10회 기준 36.4h, 행 연장 상각 33.1h). NFR-02는 **48시간 이내**(기준선 33~36시간 + 중단·복구 마진)로 확정. 미실증 개선 여지(분류 소요 17ms/타일 → 5.5ms 수준, 대역 확대)는 T13/T14에서 검증 후 반영한다. 좌표 점프 단독 경로(150시간)는 폴백으로 유지.
...
| R-12 | 눈(겨울) 지형에서 팬 추적 NCC 저하 → 간헐 오추적으로 셀 좌표 오염 | 타일 좌표 오기록 | 기대 게인 창(±80px) 밖 밴드 기각 + 2밴드 적합(T13 필수 구현 조건). 행마다 점프 재앵커로 오염 전파를 행 내로 한정. 겨울 팝업 글리프 템플릿 보강(T14) 후 표본 검증 |
